...
|구분|설명|
|----|------------|
|1. Authorization Code Grant <br/> (권한 부여 승인 코드 방식)|- 권한 부여 승인을 위해 자체 생성한 Authorization Code를 전달. <br/>- 많이 쓰이고 기본이 되는 방식 <br/>- 클라이언트가 사용자를 대신하여 특정 자원에 접근을 요청할 때 사용. 보통 타사의 클라이언트에게 보호된 자원을 제공하기 위한 인증에 사용. => 간편 로그인 기능에서 사용|
